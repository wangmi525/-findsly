...
export default function OutreachPage() {
  ...
  const channelFormats = ["email"].includes(channel);
  const channelLinks = !channelFormats;

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Outreach</h1>
        <p className="text-sm text-gray-500">Send messages to your contacts via any channel</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <h3 className="mb-3 font-semibold text-gray-900">1. Select Contacts ({selected.length})</h3>
          <div className="mb-3 max-h-80 overflow-y-auto space-y-1">
            {contacts.map(c => (
          ...
          <h3 className="mb-3 font-semibold text-gray-900">2. Choose Channel</h3>
          ...
          {channelLinks && (
            <div className="space-y-3">
              <textarea placeholder="Message (will open in app)..." value={message} onChange={e => setMessage(e.target.value)} rows={4} className="w-full rounded-lg border border-gray-200 px-3 py-2 text-sm" />
            </div>
          )}
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button onClick={sendMessage} disabled={loading || selected.length === 0 || (!channelLinks && !message)} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-500 disabled:opacity-50">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-5">
          <h3 className="mb-3 font-semibold text-gray-900">Preview</h3>
          <div className="rounded-lg border border-gray-100 bg-gray-50 p-4">
            {selected.length === 0 ? (
              <p className="text-sm text-gray-400">Select contacts to preview messages</p>
            ) : (
              <div className="space-y-2 text-sm">
                <p className="text-xs text-gray-400">To: {selected.map(c => c.name).join(", ")}</p>
                <p className="text-xs text-gray-400">Channel: {channel.toUpperCase()}</p>
                {subject && <p className="font-semibold text-gray-900">{subject}</p>}
                <p className="whitespace-pre-wrap text-gray-700">{message || "(Will generate AI message on send)"}</p>
              </div>
            )}
          </div>
        </div>
